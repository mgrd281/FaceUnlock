...
| **A spoof presented at the lock screen specifically** | A blink is required: a closure followed by a reopening, observed during that attempt. A still image cannot produce one at any distance or angle. The identity threshold, the liveness floor and every stale-feed disqualifier apply unchanged. | **A video replay of you blinking defeats this.** Binding the blink to an unpredictable moment would stop it, but that needs a prompt on the lock screen and macOS renders none for third-party plugins; an active screen flash was investigated and rejected because it needs private brightness APIs. Passive-only liveness was measured first and failed outright — the same phone scored 0.74 and 0.95 on texture isotropy at two distances, and a photograph unlocked a real Mac three times out of three. See `KNOWN_LIMITATIONS.md` §14. |

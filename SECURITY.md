...
| **A spoof presented at the lock screen specifically** | The lock-screen path enforces the identity threshold, a full liveness window before any conclusion, and every spoof disqualifier (frozen feed, repeated frames, stale timestamps). | **Weaker than every other path in the app, deliberately.** The interactive challenge — the one signal a photograph or a recording cannot answer — is not available here: the lock screen covers every window FaceUnlock owns, so a prompt has nowhere to appear. The marginal liveness band is therefore allowed through and only the floor is enforced. This is the highest-value target in the app and the one place a check is relaxed rather than refused. See `KNOWN_LIMITATIONS.md` §14. Unimplemented alternatives: a longer attempt so passive evidence reaches the band unaided, or drawing the prompt inside SecurityAgent. |

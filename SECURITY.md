...
| **Another person who resembles the enrolled user** | Per-user calibrated threshold that can only ever be stricter than the preset floor; the score is the mean of the best three enrolled samples, not the single best; several consecutive frames must match. | Identical twins and very close siblings are a genuine risk. The descriptor is a general-purpose image descriptor plus landmark geometry, not a metric-learned face network, so its discriminative power is lower than a purpose-trained model's. |
